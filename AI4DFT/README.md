...
- **`derivations.html`** — the grounding layer: notation dictionary plus every formula in lessons
  2-6 derived from scratch — radial equation, Gaussian integrals, `Hc = ESc`, the Hartree potential
  from Gauss's law, the double-counting correction, functional derivatives and LDA exchange, the
  plane-wave matrix elements and the zone-edge gap — with the numbers worked out by hand so you
  predict each script's output before running it
